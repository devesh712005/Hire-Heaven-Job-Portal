...
export const createCompany = TryCatch(
  async (req: AuthenticatedRequest, res) => {
    ...
    const { name, description, website } = req.body; //name of company
    ...
    const { data } = await axios.post(
      ...
      { buffer: fileBuffer.content }, // send return in url
    );
    const [newCompany] = await sql`
    INSERT INTO companies (name,description,website,logo,logo_public_id,recruiter_id) 
    VALUES
    (${name},${description},${website},${data.url},${data.public_id},${req.user?.user_id})
    RETURNING *
    `;
    res.json({
...
export const updateJob = TryCatch(async (req: AuthenticatedRequest, res) => {
  ...
  SELECT posted_by_recruiter_id FROM jobs WHERE 
...
export const getAllActiveJobs = TryCatch(async (req, res) => {
  const { title, location } = req.query as JobQuery;
  let queryString = `SELECT j.job_id,j.title,j.description,j.salary,j.location,j.job_type,j.role,j.work_location,j.created_at,c.name AS company_name , c.logo AS company_logo,c.company_id AS company_id 
...
export const getAllApplicationForJob = TryCatch(
  async (req: AuthenticatedRequest, res) => {
    ...
    if (user.role !== "recruiter") {
      throw new ErrorHandler(
        403,
        "Forbidden: Only recruiter can access this route",
      );
    }
