...
export const updateUserProfile = TryCatch(
  async (req: AuthenticatedRequest, res) => {
    ...
    `;
    //The database returns the updated row immediately to the backend.
    res.json({
...
export const updateProfilePic = TryCatch(
  async (req: AuthenticatedRequest, res) => {
    ...
    const { data: uploadResult }: any = await axios.post(
      //Take "data" from the response and store it in variable uploadResult
...
export const deleteSkillFromUser = TryCatch(
  async (req: AuthenticatedRequest, res) => {
    ...
    res.json({
      message: `Skill ${skillName.trim()} was deleted succesfully`,
    });
  },
);
...
export const applyForJob = TryCatch(async (req: AuthenticatedRequest, res) => {
  ...
  if (user.role !== "jobseeker") {
    throw new ErrorHandler(403, "Forbidden you are not allow for this api");
  }
  ...
  const subTime = req.user?.subscription
    ...
    : 0;
  const isSubscribed = subTime > now; //subscription_expiry > current_time

  let newApplication;
  try {
    [newApplication] =
      await sql`INSERT INTO application (job_id,applicant_id,applicant_email,resume,subscribed) VALUES ${job_id} , ${applicant_id},${user?.email},${resume},${isSubscribed} `;
  } catch (error: any) {
    if (error.code === "23505") {
